...
- Hidasi et al., *Session-based Recommendations with Recurrent Neural Networks* (GRU4Rec).
- Kang & McAuley, *Self-Attentive Sequential Recommendation* (SASRec).
- Li et al., *A Contextual-Bandit Approach to Personalized News Article Recommendation* (LinUCB).
- Industry blogs on two-tower retrieval, **monolithic vs modular** ranking, and **feature platforms** (Netflix, Uber, DoorDash tech blogs—use for citations in real interviews).
