...
- **ReAct** pattern (Yao et al., 2022) — Before ReAct, LLMs either reasoned in chain-of-thought (no grounding in reality) or used tools blindly (no reasoning about when/why). ReAct interleaves reasoning traces ("I need to search for X because...") with grounded actions (API calls, tool use), enabling the model to plan, observe results, and adapt. This is the foundational architecture for agentic systems because it solves the hallucination problem — the model verifies its reasoning against real-world observations instead of confabulating.
- **Function / tool calling** in modern LLM APIs — OpenAI and Anthropic introduced structured tool calling (JSON schema definitions, parallel tool invocation, refusal handling) to replace fragile regex-based output parsing. Understanding the function calling protocol is essential for agent system design: it determines how reliably agents can interact with external systems, how errors are surfaced, and how multi-step tool chains are orchestrated without losing context.
- **MicroVM** isolation (**Firecracker**, **gVisor**) — When agents generate and execute code, that code is untrusted by definition. Firecracker (AWS Lambda's foundation) provides VM-level isolation with <125ms boot time, while gVisor intercepts syscalls in userspace. Choosing the right isolation level (container vs. gVisor vs. microVM) determines the security vs. latency trade-off for code execution agents — a critical architectural decision when models can run arbitrary programs.
- **Distributed systems** primitives — Agent fleets amplify every distributed systems challenge: timeouts (LLM calls take seconds, not milliseconds), idempotency (retried tool calls must not duplicate side effects), backpressure (a slow tool blocks the entire agent loop), and bulkheads (one runaway agent must not exhaust shared resources). Designing agent infrastructure requires applying these primitives more aggressively than stateless API services.
- **Multi-agent** coordination — When multiple agents collaborate, the “chatroom of models” anti-pattern (agents freely messaging each other) leads to unbounded token consumption, circular reasoning, and conflicting actions. Production multi-agent systems require explicit message schemas, a supervisor agent that routes tasks, well-defined handoff protocols, and conflict resolution policies. Understanding these coordination patterns is the difference between a demo and a production system.
- **Evaluation** — Agent evaluation is fundamentally harder than LLM evaluation because agents take multi-step actions with real-world side effects. Metrics must cover task rubrics (did the agent complete the goal?), tool-use accuracy (did it call the right tools with correct arguments?), and long-horizon success (does it recover from errors gracefully?). Interviewers will ask how you detect and measure regression in agent capabilities — standard LLM benchmarks are insufficient.
