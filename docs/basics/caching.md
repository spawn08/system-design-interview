...
- [Redis Documentation](https://redis.io/docs/) — Redis provides data structures (strings, hashes, sorted sets, streams, HyperLogLog) as first-class network primitives, making it far more than a key-value cache. Understanding sorted sets (leaderboards), pub/sub (real-time notifications), and Redis Streams (lightweight event log) is essential because these patterns appear in nearly every system design question.
- [Memcached Wiki](https://github.com/memcached/memcached/wiki) — Memcached's deliberate simplicity (no persistence, no data structures beyond key-value, no replication) makes it the fastest pure cache. The wiki explains its slab allocator (how memory is managed to avoid fragmentation), consistent hashing for client-side sharding, and the trade-off of choosing Memcached over Redis: raw throughput vs. feature richness.
- *Designing Data-Intensive Applications* by Martin Kleppmann — Chapter 5: Replication — Covers the fundamental cache consistency challenge: when data is replicated (whether in a database replica or a cache layer), reads can return stale data. Kleppmann explains read-after-write consistency, monotonic reads, and the replication lag patterns that directly determine whether your cache invalidation strategy is correct.
- [Facebook's Scaling Memcache](https://research.facebook.com/publications/scaling-memcache-at-facebook/) — Facebook's 2013 paper describes operating the world's largest Memcached deployment (trillions of requests/day). It introduced regional pools, lease-based thundering herd prevention, and cross-region invalidation via McSqueal (tailing MySQL binlogs). The paper is the canonical reference for understanding caching at massive scale — how to handle hot keys, cold starts, and cache-database consistency.
- [AWS ElastiCache Best Practices](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/BestPractices.html) — Practical operational guidance on cluster sizing, connection pooling, eviction policies, and failover behavior. Particularly useful for understanding the difference between Cluster Mode Enabled (hash-slot sharding for horizontal scaling) and Cluster Mode Disabled (single-shard for simplicity) — a decision that affects every Redis-based system design.
- [Cache Stampede Prevention Techniques](https://en.wikipedia.org/wiki/Cache_stampede) — When a popular cache key expires, hundreds of concurrent requests simultaneously hit the database to regenerate it, potentially causing a cascading failure. This article explains the three prevention strategies: external recomputation (background refresh), locking (only one request regenerates), and probabilistic early expiration (XFetch — randomize TTLs so keys expire gradually instead of all at once).
